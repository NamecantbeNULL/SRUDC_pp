...
class Downsample(nn.Module):
    def __init__(self, n_feat_in, n_feat_out):
        ...
        self.body = nn.Sequential(nn.Conv2d(n_feat_in, n_feat_in//2, kernel_size=3, stride=1, padding=1, bias=False),
                                  nn.PixelUnshuffle(2))
...
class DynamicFusion(nn.Module):
	def __init__(self, in_nc=32, out_nc=64, kpn_sz=5):
		super(DynamicFusion, self).__init__()
		self.dynamic_kernel = nn.Sequential(
			nn.Conv2d(in_nc, out_nc, 1),
			nn.LeakyReLU(0.2, inplace=True),
			nn.Conv2d(out_nc, out_nc * (kpn_sz ** 2), 1))
		self.bias = nn.Sequential(
			nn.Conv2d(in_nc, out_nc, 1),
			nn.LeakyReLU(0.2, inplace=True),
			nn.Conv2d(out_nc, out_nc, 1))
	...
	def forward(self, feats):
		...
		dynamic_kernel = self.dynamic_kernel(y)
		bias = self.bias(y)
		out = kernel2d_conv(x, dynamic_kernel, self.kpn_sz) + bias
		return out
...
class SRUDC(nn.Module):
	def __init__(self, kernel_size=5, base_dim=32, depths=[4, 4, 4, 4, 4, 4, 4], conv_layer=ConvLayer, norm_layer=nn.BatchNorm2d, gate_act=nn.Sigmoid, fusion_layer=SKFusion):
		...
		for i in range(self.half_num):
			...
			# self.fusions_cond.append(DynamicFusion(in_nc=embed_dims[i], out_nc=embed_dims[i], kpn_sz=5))
			self.fusions_cond.append(AttentionFusion(embed_dims[i], num_heads=4, bias=False))
